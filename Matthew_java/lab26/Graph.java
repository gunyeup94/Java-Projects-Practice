...
public class Graph {
    ...
    public Graph prims(int start) {
        ...
        primTree.addVertex(start);
        Set<Integer> visited = new HashSet<>();
        PriorityQueue<Integer> currVertices = new PriorityQueue<>();
        currVertices.add(start);

        int count = 0;
        while(count < getAllVertices().size() - 1) {
            for (Edge e : getEdges(start)) {
                if (!visited.contains(e.getDest())) {
                    currPQ.add(e);
                }
            }
            Edge smallestEdge = currPQ.poll();
            if (smallestEdge != null) {
                if (!visited.contains(smallestEdge.getDest())) {
                    if (!(getEdges(smallestEdge.getDest()).size() == 1)) {
                        visited.add(start);
                    }
                    primTree.addVertex(smallestEdge.getDest());
                    primTree.addEdge(start, smallestEdge.getDest(), smallestEdge.getWeight());
                    visited.add(smallestEdge.getDest());
                    start = smallestEdge.getDest();
                    count++;
                }
            }
        }
    ...
    public Graph kruskals() {
        Graph K = new Graph();
        Set<Integer> visited = new HashSet<>();
        for (Integer x : getAllVertices()) {
            K.addVertex(x);
        }
        for (Edge x : getAllEdges()) {
            if (!(visited.contains(x.getSource()) && visited.contains(x.getDest()))) {
                K.addEdge(x.getSource(), x.getDest(), x.getWeight());
                visited.add(x.getSource());
                visited.add(x.getDest());
            }
        }
    ...
    public static void main(String[] args) {
        Graph test1 = Graph.loadFromText("graphTestNormal.in");
        Graph test = test1.prims(0);
        Graph testResult = Graph.loadFromText("graphTestResult.in");
        System.out.println(test.equals(testResult));
    }
}
