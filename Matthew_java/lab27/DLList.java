...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        if (sentinel.next == sentinel && sentinel.prev == sentinel) {
            ...
            for (Node curr = sentinel.next; curr.item != null; curr = curr.next) {
                if (curr.item.compareTo(item) > 0) {
                    if (curr == sentinel.next) {
                        addFirst(item);
                        return;
                    } else {
                        Node newInsert = new Node(item, curr.prev, curr);
                        curr.prev.next = newInsert;
                        curr.prev = newInsert;
                        return;
                    }
                } else if (curr.next.item == null) {
                    addLast(item);
                    return;
                }
            }
    ...
    public DLList<T> mergeSort() {
        ...
        int halfCount = 0;
        while (halfCount < size / 2) {
            oneHalf.addFirst(curr.item);
            curr = curr.next;
            halfCount++;
        }
        while (halfCount < size) {
            otherHalf.addFirst(curr.item);
            curr = curr.next;
            halfCount++;
        }
    ...
    public DLList<T> quicksort() {
        ...
        while (curr.item != null) {
            ...
            } else if (curr.item.compareTo(pivot) == 0) {
                equalElements.addLast(curr.item);
            } else {
